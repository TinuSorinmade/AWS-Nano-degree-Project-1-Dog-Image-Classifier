...
def check_calculating_results(results_dic, results_stats_dic):
    ...
    if results_stats_dic is None:
        ...
        # Iterate through results_dic dictionary to recompute the statistics
        # outside of the calculates_results_stats() function
        for key in results_dic:
            # Check if the list associated with the key has at least 5 elements
            if len(results_dic[key]) >= 5:
                ...
                print(f"Warning: Insufficient elements in results_dic[{key}]")

        # Calculate statistics based upon counters from above
        ...
        print(f"N Images: {results_stats_dic.get('n_images', 0)}  N Dog Images: {results_stats_dic.get('n_dogs_img', 0)}  N NotDog Images: {results_stats_dic.get('n_notdogs_img', 0)} \nPct Corr dog: {results_stats_dic.get('pct_correct_dogs', 0):5.1f} Pct Corr NOTdog: {results_stats_dic.get('pct_correct_notdogs', 0):5.1f}  Pct Corr Breed: {results_stats_dic.get('pct_correct_breed', 0):5.1f}")
